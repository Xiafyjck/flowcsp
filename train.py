...
def parse_args():
    ...
    parser.add_argument('--train_path', type=str, 
                        default='data/train_cache',
                        help='Path to training memmap cache directory (generated by warmup_cache.py)')
    parser.add_argument('--val_path', type=str,
                        default='data/val_cache',
                        help='Path to validation memmap cache directory (generated by warmup_cache.py)')
...
def main():
    ...
    # 创建数据模块
    print(f"📁 Loading memmap cache data:")
    print(f"  Training: {args.train_path}")
    print(f"  Validation: {args.val_path}")
    if args.test_path:
        print(f"  Test: {args.test_path}")
    
    # 验证缓存目录存在
    if not Path(args.train_path).exists():
        print(f"❌ Training cache not found: {args.train_path}")
        print(f"   Please run: python scripts/warmup_cache.py --csv your_train.csv --output_dir {args.train_path}")
        return
